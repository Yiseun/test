...
const RentCarLike = () => {
  ...
  const perPage = 8;
  const count = Math.ceil(rentCars.length / perPage);
  const rentCarListsPerPage = usePagination(rentCars, perPage);
  ...
  function searchAllAttraction() {
    axios
      .get(BASE_URL + "/api/auth/v1/user/like/rentcar/list", {
        headers: {
  ...
  return (
    <ThemeProvider theme={theme}>
      <Container maxWidth="lg">
        ...
        <Box display="flex">
          <DashboardMyInfo page="RENTCAR" />
          <Wrapper
            name="MY RENTCAR LIKE"
            onClick={() => {
